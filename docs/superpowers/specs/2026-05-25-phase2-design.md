...
- **Edge cases**:
  ...
  - 二重 `start()` → `RuntimeError` または冪等動作 (どちらでもよいが挙動を固定)
  - `stream_chat()` 実行中に外部から `stop()` → 進行中の Conversation を `__aexit__` で正常終了
...
- **Input**: Mock を「最初の chunk 送出後に例外を上げる」モードで起動、HTTP `POST` with `stream:true`
- **Expected**:
  - HTTP ステータス `200` (SSE 開始済みのため)
  - 0 個以上の通常 chunk フレーム
  - `data: {"error":{"type":"server_error","message":"..."}}\n\n` フレーム 1 個
  - `data: [DONE]\n\n` で終端
  - HTTP 接続が close
...
- **Edge cases**:
  - chunk が 1 個も送出されていない段階でエラー → HTTP ヘッダ未送信ならステータスコード (4xx/5xx) + OpenAI 形式 JSON 本体で返す (SSE モードに切り替えない)
...
2. **messages 配列の一括渡し API**: Section 3.3 の前提 (ステートレス・一括渡し) を成立させる SDK API が存在するかを確定させる。候補は `Conversation.chat(messages=[...])`, `Agent.chat(messages=[...])`, `history` 引数経由のいずれか。**もし一括渡し API が存在しない場合は、Section 3.3 のステートレス方針を維持するために代替手段 (例: リクエストごとに Agent 再起動) を再検討する**
