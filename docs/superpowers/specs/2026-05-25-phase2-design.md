...
### 3.3 Agent ライフサイクル (長寿命 Agent + 都度 Conversation)
...
[Each chat.completions request]
  └─ AntigravityClient.stream_chat(messages)
      └─ Conversation.create(strategy).__aenter__()       ← 都度
          ├─ 過去 messages を順次 send (応答は破棄)
          └─ 最後の user message で chat() → token yield
...
|---|---|---|
| `src/server.ts` | `createServer(backend)` | `POST /v1/chat/completions` で `stream:true` 検出時に `Content-Type: text/event-stream` で応答開始。`JsonRpcClient` の chunk callback を SSE フレーム (`data: {...}\n\n`) として書き出し、最終 response 受信時に `data: [DONE]\n\n` を送出して end。`stream:false` は従来通り 1 発の JSON 応答。 |
...
23. `test_chat_completions_async_generator`: AsyncGenerator として token を yield することの検証
24. `test_chat_completions_stream_false_aggregates`: stream:false で内部集約して 1 発返すことの検証
25. `test_antigravity_client_lifecycle`: AntigravityClient の `start()` / `stop()` / `stream_chat()` のライフサイクル検証

#### TS 単体 (2 ケース追加)

26. `parses notification and dispatches to onChunk by request_id`: Notification の request_id への dispatch 検証
27. `notification with unknown request_id is ignored with warning`: 未知 request_id の Notification 破棄検証

#### TS 統合 (2 ケース追加)

28. `streamingCall round-trips chunks and final response`: stdio 経由で Notification → 最終 response の順序検証
29. `idle timeout fires when chunks stop arriving`: アイドルタイムアウト発火と Pending Map クリーンアップ検証

#### E2E (2 ケース追加)

30. `POST /v1/chat/completions with stream:true returns SSE stream with [DONE]`: SSE 出力フォーマットとチャンク順序検証
31. `POST /v1/chat/completions with stream:true and SDK error returns SSE error frame`: ストリーミング中エラーの SSE フレーム検証
...
- 将来 nightly ワークフロー (Bitbucket Pipelines) で 1 日 1 回実行する運用も可能
...
2. **`Agent.chat()` の messages 受け取り API**: 単一テキストか messages 配列か。`Conversation` の `send()` が「応答なし送信」を許すかを確認
...
2. `pnpm test:e2e:live` が手動実行で PASS (実 Gemini API 経由)
