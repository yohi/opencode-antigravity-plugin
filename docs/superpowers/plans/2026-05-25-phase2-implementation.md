...
class MockAntigravityClient:
    ...
    async def stream_chat(
        ...
        try:
            # mock_options は T4.2 のエラー注入で利用する。通常 mock では None。
            _ = mock_options
...
class AntigravityClient:
    ...
    async def stream_chat(
        ...
        if self._agent is None:
            raise RuntimeError("client is not running")
        # live 実装では mock_options を無視する。Protocol 署名を Mock と揃え、
        # live mode に _mock が届いても TypeError にしないための引数。
        _ = mock_options
...
- [ ] **Step 6: pyproject.toml に optional dep を追加し、uv.lock を更新**
...
- 設計書 Section 6 (エラー) → T2.1, T3.2, T3.5, T4.2。**6.4 二重検証 → T3.1.5 (一次) + T2.5 (二次)**
...
- 設計書 Section 12.4 (Phase 2.5 cancel 申し送り) → Phase 2 範囲外 (T3.5 Step 3 で best-effort のみ実装)
...
- 全 19 タスク (T0.1, T0.2, T1.1, T2.1〜T2.6, T2.3.1, T3.1, T3.1.5, T3.2〜T3.5, T4.1〜T4.3, T5.1) の Step 1 に `git merge-base --is-ancestor` 検証を埋め込み済み
