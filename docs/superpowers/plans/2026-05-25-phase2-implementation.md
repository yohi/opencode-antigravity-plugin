...
- [x] **Step 2: `devcontainer.json` に Phase 2 既定環境変数を追加**
...
- [x] **Step 3: Dockerfile のバージョン pin 確認**
...
- [x] **Step 4: devcontainer 内で `printenv` で環境変数注入を検証**
...
- [x] **Step 2: 既存 `.github/workflows/ci.yml` を読み、差分箇所を特定**
...
- [x] **Step 3: `.github/workflows/ci.yml` を Phase 2 用に書き換える**
...
- [x] **Step 4: YAML 構文を devcontainer 内で検証**
...
- [x] **Step 5: actionlint で GitHub Actions 構文を静的検証 (任意だが推奨)**
...
- [x] **Step 6: 既存 `pnpm verify` がローカル devcontainer で通ることを最終確認**
...
- [x] **Step 9: Draft PR URL を `.stack-urls.md` に追記**
...
- [x] **Step 2a: 失敗するテストを書く: `tests/python/unit/test_prompt_folding.py` (受け入れ#32)**
...
- [x] **Step 2b: 失敗するテストを書く: `tests/python/unit/test_antigravity_client.py` (受け入れ#25 改訂)**
...
- [x] **Step 3: pytest-asyncio を dev-deps に追加**
...
- [x] **Step 5a: `prompt_folding.py` を実装 (設計書 Section 3.3.1)**
...
- [x] **Step 5b: `antigravity_client.py` を実装 (per-request Agent + Mock)**
...
- [x] **⚠️ 重要: Step 6: pyproject.toml に optional dep を追加し、uv.lock を更新**
...
- [x] **Step 4: `src/errors.ts` に SDK エラーコードマッピングを追加**
...
**派生元ブランチ:** `feature/phase2/ts-types` (T3.1) — 実行時は T3.1 が master にマージ済みのため `origin/master` へスタックを畳んで派生
...
cd /workspaces/opencode-antigravity-plugin
git fetch origin master --no-tags
git switch -c feature/phase2/ts-jsonrpc-streaming origin/master
...
- [x] **Step 2: 失敗するテストを書く (受け入れ#26, #27)**
...
**進捗メモ (2026-05-31):** T3.3 完了。PR #48 (Draft) 作成済み → https://github.com/yohi/opencode-antigravity-plugin/pull/48
派生元は master (T3.1 PR #38 が master に merge 済みのため、計画書の `派生元: T3.1` のスタックを `origin/master` に畳んだ)。
...
- [x] **Step 2: T2.6 (Python 全 Phase 2 完了の最終 PR) を取り込む**

T2.6 は master に merge 済みのため、追加 merge は不要。T4.2 は `feature/phase2/ts-server-sse` から派生。
...
- [x] **Step 3: E2E テストを書く (受け入れ#30, #31)**
...
- [x] **Step 4: TS と Mock に Header ベースのエラー注入機構を追加**
...
実行結果: `tests/ts/sse.test.ts` 2 cases PASS (`pnpm verify` 内の `pnpm test:e2e`)。

- [x] **Step 6: 総合検証 (受け入れ基準 #1)**
...
実行結果: `pnpm verify` PASS。Python tests は 80 passed / 2 skipped、TS unit 55 passed、TS integration 7 passed、TS E2E 6 passed。
...
- `49263ef feat(python): streaming mock options をハンドラへ連携`
- `2e8aa3c feat(e2e): SSE mock error 経路のE2Eを追加`
- `9011ce1 test(e2e): Python ワーカ起動系テストを直列化`
- `dd4142c test(python): invalid UTF-8 E2Eを起動完了後に実行`
...
Draft PR: https://github.com/yohi/opencode-antigravity-plugin/pull/51
