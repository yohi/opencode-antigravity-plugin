...
| T1.1 | SDK API スパイク調査 | `master` | 並列可能 (T2.3.1 のブロッカー) |
| T2.1 | Python `errors.py` 新設 | `master` | 並列可能 (独立) |
| T2.2 | Python `format_notification` 追加 | `master` | 並列可能 (独立) |
| T2.3 | Python `antigravity_client.py` 新設 (Mock + stub) | `master` | 並列可能 (独立) |
| T2.3.1 | Python `AntigravityClient` 本実装 (T1.1 結果反映) | `master` (T2.3 マージ後) | 直列必須 (Wait for T1.1 + T2.3 merged) |
...
**派生元ブランチ:** `master`
**実行モード:** 並列可能 — T2.1 / T2.2 / T3.x / T4.1 と同時実行可。ただし **T1.1 (SDK スパイク) が未完了でも本タスクは進められる**: Mock 部分のみ完成させ、`AntigravityClient` 本体は `NotImplementedError` の stub にとどめる。T1.1 完了後に **必ず T2.3.1 (フォローアップ) で stub を実装に置き換える**。
**前提条件:** なし (T1.1 の完了は望ましいが必須ではない)。T1.1 結果が出ている場合は実 API シグネチャを直接反映する。
...
### Task 2.3.1: `AntigravityClient` stub を T1.1 結果で本実装に置き換える (フォローアップ)

**派生元ブランチ:** `feature/phase2/python-antigravity-client` (T2.3) もしくは master に T2.3 がマージ済みであれば master
**実行モード:** 直列必須 — **T1.1 と T2.3 の両方が完了** していること
**前提条件:**

- T1.1 の Draft PR URL が `.stack-urls.md` に記録されており、`docs/superpowers/specs/2026-05-25-phase2-sdk-spike.md` に採用 API・例外型・cold-start 計測値が確定済み
- T2.3 がマージされている (本タスクは T2.3 の本実装パートのみを差し替える性質のため)

**Files:**

- Modify: `backend/src/opencode_antigravity/antigravity_client.py` (`AntigravityClient.stream_chat` 実装)
- Modify: `backend/src/opencode_antigravity/errors.py` (T2.1 で仮置きした例外型名を T1.1 結果に同期)
- Create / Modify: `tests/python/unit/test_antigravity_client_live.py` (live マーカ付きの最低 1 ケース)
...
cd /workspaces/opencode-antigravity-plugin
git fetch origin master --no-tags
git switch -c feature/phase2/python-antigravity-client-impl origin/master
...
- [ ] **Step 2: T1.1 で採択された API を読み込む**

`docs/superpowers/specs/2026-05-25-phase2-sdk-spike.md` の Section 2「採用 API」を確認。以下のいずれかが採用されているはず (設計書 3.3.1 の優先順位):

- (a) `Conversation.chat(messages=[...])`
- (b) `Agent.chat(messages=[...])`
- (c) `Conversation.chat(prompt, history=[...])` (history 引数経由)
- (d) リクエストごとに Agent 再起動 (cold-start < 100ms を満たす場合のみ)

- [ ] **Step 3: `AntigravityClient.stream_chat` を本実装に置き換える**

T2.3 で仮置きしていた以下のスニペットを採用 API に応じて差し替える:

```python
# 旧 (T2.3 の仮実装)
async with self._agent.conversation() as conv:
    async for token in conv.chat(messages=messages):
        yield str(token)
```

採用 API ごとの実装例 (T1.1 の結果に合わせて 1 つを選択):

```python
# (a) Conversation.chat(messages=[...]) を採用した場合
async with self._agent.conversation() as conv:
    async for token in conv.chat(messages=messages):
        yield str(token)

# (b) Agent.chat(messages=[...]) を採用した場合
async for token in self._agent.chat(messages=messages):
    yield str(token)

# (c) history 引数経由を採用した場合
*history, last_user = messages
async with self._agent.conversation() as conv:
    async for token in conv.chat(prompt=last_user["content"], history=history):
        yield str(token)

# (d) Agent 再起動フォールバックの場合 (cold-start < OAG_AGENT_COLDSTART_BUDGET_MS が前提)
from google.antigravity import Agent, LocalAgentConfig  # type: ignore[attr-defined]
async with Agent(LocalAgentConfig(model=self.model)) as agent:
    async for token in agent.chat(messages=messages):
        yield str(token)
```

- [ ] **Step 4: T2.1 の例外型を T1.1 結果に同期**

`errors.py` の `SdkError` サブクラスにマップしている SDK 例外型名 (`AuthenticationError` 等の仮置き) を、スパイク結果の実型名に置換する。`isinstance` チェック箇所も漏れなく更新する。

- [ ] **Step 5: live マーカ付きの最低限テストを追加**

```python
# tests/python/unit/test_antigravity_client_live.py
import os
import pytest
...
async def test_live_stream_chat_smoke():
    ...
    if not api_key:
        pytest.skip("GEMINI_API_KEY not set; skipping live smoke")
    client = AntigravityClient(model=os.environ.get("ANTIGRAVITY_MODEL", "gemini-2.5-pro"), api_key=api_key)
    await client.start()
    try:
        chunks: list[str] = []
...
- [ ] **Step 6: mock CI が壊れていないことを確認**

```bash
OAG_BACKEND_MODE=mock ANTIGRAVITY_MODEL=gemini-2.5-pro uv run pytest tests/python -m "not live"
```

Expected: 全 PASS (live マーカは skip される)。

- [ ] **Step 7: コミット**

```bash
git add backend/src/opencode_antigravity/antigravity_client.py \
        backend/src/opencode_antigravity/errors.py \
        tests/python/unit/test_antigravity_client_live.py
git commit -m "feat(python): AntigravityClient 本実装 (T1.1 スパイク結果反映) + live smoke テスト"
```

- [ ] **Step 8: プッシュと Draft PR 作成、URL を記録**

```bash
git push -u origin feature/phase2/python-antigravity-client-impl
gh pr create --draft --base master \
  --title "feat(python): AntigravityClient stub を T1.1 結果で本実装に置き換え" \
  --body "T2.3 で stub にしていた AntigravityClient.stream_chat を T1.1 採用 API で実装。live smoke テスト追加。"
```
...
- 設計書 5.1.1 のパターンに従い、**`async for` ループを使わず `__anext__()` ループ + `StopAsyncIteration.value` で最終戻り値を取得する**。`async for` は内部で `StopAsyncIteration` を握り潰すため戻り値が取れない。
...
if inspect.isasyncgen(result) and params.get("stream") is True:
    ...
    try:
        while True:
            ...
            writer.write(format_notification("chat.completions.chunk", {"request_id": req_id, "delta": delta}).encode("utf-8"))
            await writer.drain()
    except StopAsyncIteration as stop:
        final_result = stop.value or {}
    finally:
        await agen.aclose()
    writer.write(format_response(req_id, final_result).encode("utf-8"))
    await writer.drain()
    ...
    writer.write(format_response(req_id, response_dict).encode("utf-8"))
    await writer.drain()
```
...
async def test_chat_completions_async_generator_stream_true():
    """設計書 5.1.1 の StopAsyncIteration.value パターンで最終戻り値を取得する。"""
    client = MockAntigravityClient(model="gemini-2.5-pro")
    await client.start()
    try:
        params = {
        ...
        # _stream_impl の return 値 (内部メタ: finish_reason + usage)
...
- `chat_completions(params, client)` を **2 つのオーバーロード相当** で実装する:
  - `params["stream"] == True` → AsyncGenerator を返す (yield: `{"delta": ...}` のみ、return: **内部メタ `{"finish_reason", "usage"}` のみ**)
...
async def _aggregate_impl(cp: _ChatParams, client: AntigravityClientBase) -> dict:
    # 設計書 5.2 / 5.2.1: stream:false でも内部はストリーミングし上限まで集約する
    max_tokens = int(os.environ.get("OAG_MAX_AGGREGATE_TOKENS", "8192"))
    buf: list[str] = []
    completion_tokens = 0
    agen = client.stream_chat([m.model_dump() for m in cp.messages])
    try:
...
- 設計書 8.1 の以下の任意環境変数を起動時にログ出力 (validation はしない、値を `os.environ.setdefault` でデフォルト保証してもよい):
  ...
  - `OAG_AGENT_COLDSTART_BUDGET_MS` (既定 100) — T1.1 で参照
...
- 最終 response 受信時: `clearTimeout(entry.idleTimeoutId)` と `clearTimeout(entry.globalTimeoutId)` の両方を呼んでから resolve / Pending Map から削除
...
- リクエスト受信 → JSON ボディ解析 → Pydantic 検証 → `backend.streamingCall` を発行する **直前** までを Phase A とする
- Phase A の検証エラーは **HTTP 4xx + OpenAI 形式 JSON** で即時応答 (`{"error":{"type":..., "message":...}}`)。SSE モードに切り替えない
...
- `writeSseChunk(delta, options)` の責務 (設計書 4.3.1):
  ...
  - 終端で `data: [DONE]\n\n` を送出して `res.end()`
- Phase B 内で発生した全ての例外 (SDK エラー、タイムアウト、Python クラッシュ) を catch:
...
if (stream) {
  ...
  let validated;
  try {
    validated = ChatCompletionsParamsSchema.parse(req.body);
  } catch (e) {
    return res.status(400).json({ error: { type: "invalid_request_error", message: String(e) } });
  }
...
mock_opts = params.get("_mock") if isinstance(params, dict) else None
async for tok in client.stream_chat([m.model_dump() for m in cp.messages], _mock=mock_opts):
    # ... 既存ロジック

# antigravity_client.py の MockAntigravityClient.stream_chat
async def stream_chat(self, messages, *, _mock=None):
    fail_after = (_mock or {}).get("fail_after_chunk")
    yielded = 0
    # ... 既存の token 生成ループ内で
    yielded += 1
