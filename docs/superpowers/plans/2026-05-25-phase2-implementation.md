...
`import re` と `import asyncio` を冒頭の `from __future__` の直後に追加する。
        ...
        try:
            try:
                ...
                raise SdkConnectionError(
                    f"Agent cold-start exceeded OAG_AGENT_COLDSTART_TIMEOUT_MS"
                ) from e
            except Exception as e:
            ...
            # asyncio.CancelledError / 上記 raise いずれも __aexit__ を確実に呼ぶ
            raise
...
class _SlowMockClient(MockAntigravityClient):
    ...
    async def stream_chat(self, messages, *, mock_options=None):
        # 親の enter カウント前に sleep して semaphore の効果を可視化
        ...
        try:
            ...
            async for tok in super().stream_chat(messages, mock_options=mock_options):
                yield tok
        finally:
            self.active_enter_count -= 1
...
async def test_cold_start_timeout_maps_to_sdk_connection_error(monkeypatch):
    ...
    class _NeverEnterClient(MockAntigravityClient):
        async def stream_chat(self, messages, *, mock_options=None):
            # asyncio.wait_for を MockAntigravityClient で再現
            try:
                await asyncio.wait_for(asyncio.sleep(1.0), timeout=0.05)
            except asyncio.TimeoutError as e:
                raise SdkConnectionError("Agent cold-start exceeded OAG_AGENT_COLDSTART_TIMEOUT_MS") from e
            yield "unreachable"
...
def _get_semaphore() -> asyncio.Semaphore:
    ...
    if _semaphore is None or _semaphore._value + _semaphore._waiters.__len__() != limit:
        # 値変更時は再生成 (テスト env 切替対応)
...
> **重要 (実装メモ):** `_semaphore._value + len(_semaphore._waiters)` は asyncio 内部実装に依存するため fragile。代替として、現在の値の変化を `(limit, _semaphore)` のタプルでキャッシュして比較する方が安全:
>
> ```python
