...
**Tech Stack:** Node.js 24 (現行 Active LTS) / TypeScript 5 / pnpm / vitest / pino, Python 3.13 / uv / hatchling / pydantic / pytest / ruff, Devcontainer (mcr.microsoft.com/devcontainers/python:3.13 + NodeSource setup_24.x), GitHub Actions (ubuntu-slim).
...
| Phase | 主な生成/変更ファイル |
|---|---|
| 0 | `.devcontainer/devcontainer.json`, `.devcontainer/Dockerfile`, `.gitignore`, `README.md`, `package.json`, `pnpm-lock.yaml`, `tsconfig.json`, `pyproject.toml`, `uv.lock`, `.github/workflows/ci.yml` |
...
### Task 0.3: GitHub Actions CI/CD 設定
...
# (YAML を .github/workflows/ci.yml に保存)
git add .github/workflows/ci.yml
git commit -m "ci: GitHub Actions ワークフローを追加"
git push -u origin phase-0/cicd

gh pr create --draft --base phase-0/package-init --title "Phase 0.3: GitHub Actions CI/CD" --body "$(cat <<'EOF'
## Summary
- .github/workflows/ci.yml を追加
- master ブランチ + 全 PR で `pnpm verify` を実行

## Test plan
- [ ] (master マージ後) GitHub Actions で verify ステップが PASS
EOF
)"
```
...
3. `phase-0/cicd` → master ← ここで GitHub Actions が初稼働
...
14. `phase-5/e2e-verify` → master ← 最後に master 上で `pnpm verify` がパスすることを GitHub Actions で再確認
